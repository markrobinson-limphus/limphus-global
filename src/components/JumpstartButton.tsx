...
export function JumpstartButton() {
  const [loading, setLoading] = useState(false);
  const [done, setDone] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [count, setCount] = useState<number | null>(null);

  async function handleJumpstart() {
    if (loading) return;
    setLoading(true);
    setDone(false);
    setError(null);
    setCount(null);
    try {
      const res = await fetch("/api/leads/jumpstart", { method: "POST", credentials: "include" });
  ...
  return (
    <span className="inline-flex flex-wrap items-center gap-2">
      ...
      >
        ...
        {loading ? "Running…" : done ? "Jumpstart done" : "Jumpstart pipeline"}
      </button>
      {count != null && done && (
        <span className="text-xs text-emerald-400">{count} leads set to READY</span>
      )}
